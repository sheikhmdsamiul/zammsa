...
const CPPCreate: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (!recommendedMethod && !methodOverride) { toast.error('Procurement method must be determined'); return; }
    ...
    const risksWithoutMitigation = risks.filter(r => !r.mitigation_strategy);
    if (risksWithoutMitigation.length > 0) { toast.error('All risks must have a mitigation strategy'); return; }
    ...
    try {
      ...
      const createdMethod = created.method || (methodOverride ? (newMethodOverride as ProcurementMethod) : recommendedMethod);
      if (created.status === 'approved' && ['open_tender', 'international'].includes((createdMethod || '') as string)) {
            ...
            <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              ...
              <ul className="mt-2 space-y-1 text-sm text-gray-700">
                <li className="flex items-center gap-2">
                  <span className={selectedRequisition ? 'text-green-500' : 'text-red-500'}>{selectedRequisition ? '✓' : '✗'}</span> Requisition selected
                </li>
                <li className="flex items-center gap-2">
                  <span className={(recommendedMethod || methodOverride) ? 'text-green-500' : 'text-red-500'}>{(recommendedMethod || methodOverride) ? '✓' : '✗'}</span> Procurement method determined
                </li>
                <li className="flex items-center gap-2">
                  <span className={milestones.length > 0 ? 'text-green-500' : 'text-red-500'}>{milestones.length > 0 ? '✓' : '✗'}</span> At least one milestone defined
                </li>
                <li className="flex items-center gap-2">
                  <span className={resourceRequirements.evaluationCommitteeSize >= 3 ? 'text-green-500' : 'text-red-500'}>{resourceRequirements.evaluationCommitteeSize >= 3 ? '✓' : '✗'}</span> Evaluation committee size (min 3)
                </li>
                <li className="flex items-center gap-2">
                  <span className={risks.length > 0 ? 'text-green-500' : 'text-red-500'}>{risks.length > 0 ? '✓' : '✗'}</span> At least one risk identified
                </li>
                <li className="flex items-center gap-2">
                  <span className={risks.length > 0 && risks.every(r => r.mitigation_strategy) ? 'text-green-500' : 'text-red-500'}>{(risks.length > 0 && risks.every(r => r.mitigation_strategy)) ? '✓' : '✗'}</span> All risks have mitigation strategy
                </li>
              </ul>
            </div>
          </div>
      ...
      <div className="flex justify-between pt-6 pb-8">
        ...
        <div className="flex gap-3">
          {currentStep === CPPSteps.length - 1 ? (
            ...
            >
              {loading ? 'Creating CPP...' : 'Submit CPP'}
            </button>
          ) : (
            <button
              onClick={nextStep}
